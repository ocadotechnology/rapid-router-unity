...
namespace Road
{
    public class RoadDrawer : MonoBehaviour
    {
		...
		{
			return Instantiate(tiles.deadEndRoadTile, new Vector3(coords.x, coords.y, 0f),
				Quaternion.Euler(0, 0, (float)direction)) as GameObject;
		}

		public GameObject DrawStraight(Coordinate coords, Direction direction)
		{
			return Instantiate(tiles.straightRoadTile, new Vector3(coords.x, coords.y, 0f),
				Quaternion.Euler(0, 0, (float)direction)) as GameObject;
		}

		public GameObject DrawTurn(Coordinate coords, Direction direction)
		{
			return Instantiate(tiles.turnRoadTile, new Vector3(coords.x, coords.y, 0f),
				Quaternion.Euler(0, 0, (float)direction)) as GameObject;
		}

		public GameObject DrawTJunction(Coordinate coords, Direction direction)
		{
			return Instantiate(tiles.tJunctionTile, new Vector3(coords.x, coords.y, 0f),
				Quaternion.Euler(0, 0, (float)direction)) as GameObject;
		}

		public GameObject DrawCrossRoad(Coordinate coords)
		{
			return Instantiate(tiles.crossRoadTile, new Vector3(coords.x, coords.y, 0f), Quaternion.identity) as GameObject;
		}
